    ...
    return (
        <mesh ref={meshRef}>
            ...
            <meshStandardMaterial
                color={isHovered ? "#0ea5e9" : "#6366f1"}
                emissive={isHovered ? "#0ea5e9" : "#3b82f6"}
...
export default function Navbar() {
    ...
    return (
        <motion.header
            ...
            className="fixed top-0 left-0 right-0 z-[9999] px-2 sm:px-4 lg:px-6 py-2"
        >
            <nav className="max-w-7xl mx-auto bg-white/70 dark:bg-slate-950/70 backdrop-blur-2xl border border-white/30 dark:border-white/10 rounded-full shadow-[0_10px_40px_rgba(0,0,0,0.12)] dark:shadow-[0_10px_40px_rgba(0,0,0,0.5)] transition-all duration-300">
                <div className="px-4 lg:px-8 h-14 lg:h-16 flex items-center justify-between">
                    ...
                    >
                        <div className="w-7 h-7 sm:w-8 sm:h-8 lg:w-10 lg:h-10 relative">
                            <Canvas camera={{ position: [0, 0, 4.5], fov: 45 }} style={{ background: "transparent" }}>
                                <ambientLight intensity={0.5} />
                                <pointLight position={[10, 10, 10]} intensity={1.5} />
                                <Suspense fallback={null}>
                                    <Logo3D isHovered={hoveredLogo} />
                                </Suspense>
                            </Canvas>
                        </div>
                    ...
                    <div className="hidden lg:flex items-center gap-4">
                        <ThemeToggle />
                        <button className="relative group px-6 py-2.5 rounded-full overflow-hidden font-semibold text-sm text-white shadow-lg shadow-cyan-500/20 hover:scale-105 hover:shadow-cyan-500/40 transition-all duration-300">
            ...
            <AnimatePresence>
                {isMobileMenuOpen && (
                    ...
                    >
                        {/* Drawer content */}
                        <motion.div
                            className="w-full sm:w-[400px] h-full bg-white dark:bg-[#020617] sm:border-l border-slate-200 dark:border-white/10 p-5 sm:p-6 flex flex-col"
                            initial={{ x: "100%" }}
                            animate={{ x: "0%" }}
                            exit={{ x: "100%" }}
                        ...
                        >
                            ...
                            <div className="sticky top-0 z-10 flex justify-between items-center bg-white dark:bg-[#020617] pb-4 border-b border-slate-200 dark:border-white/5 mb-6 pt-2">
                            ...
                            {/* Menu Links */}
                            <div className="flex flex-col gap-1.5 overflow-y-auto pb-6 scrollbar-hide">
                                {navLinks.map((link, i) => {
                                    ...
                                    return (
                                        <Link
                                            key={link.name}
                                            href={link.href}
                                            className={`text-lg font-medium transition-all duration-300 block px-4 py-3.5 rounded-xl active:scale-[0.98] ${isActive ? "bg-primary/10 dark:bg-cyan-500/10 text-primary dark:text-cyan-400" : "text-slate-600 dark:text-gray-300 hover:bg-slate-50 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white"}`}
                                            onClick={() => setIsMobileMenuOpen(false)}
                                        >
                                            <motion.div
                            ...
                            <motion.div
                                className="mt-auto pt-6 border-t border-slate-200 dark:border-white/5"
                                initial={{ opacity: 0, y: 20 }}
                                animate={{ opacity: 1, y: 0 }}
                                transition={{ delay: 0.4 }}
                            >
                                <Link href="/contact" className="w-full relative group px-6 py-4 rounded-xl overflow-hidden font-bold text-white shadow-lg shadow-primary/20 dark:shadow-cyan-500/20 block text-center active:scale-[0.98] transition-transform" onClick={() => setIsMobileMenuOpen(false)}>
