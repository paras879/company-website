...
/* ═══════════════════════════════════════════════════════
   UTILITY COMPONENTS
   ═══════════════════════════════════════════════════════ */
function GlassCard({ children, delay = 0, hoverColorClass = "from-primary to-accent", tilt = true, className = "" }) {
    ...
    const rotateX = useTransform(sy, [-0.5, 0.5], tilt ? ["10deg", "-10deg"] : ["0deg", "0deg"]);
    const rotateY = useTransform(sx, [-0.5, 0.5], tilt ? ["-10deg", "10deg"] : ["0deg", "0deg"]);
    ...
    return (
        <motion.div
            ...
            whileHover={{ scale: 1.02, z: tilt ? 20 : 0 }}
        >
            <div
                className={`relative p-6 rounded-2xl h-full overflow-hidden flex flex-col z-10 bg-white/50 dark:bg-white/5 border border-slate-200 dark:border-white/10 backdrop-blur-xl shadow-premium transition-all duration-400`}
            >
                {/* Border trace animation on hover */}
                <motion.div
                    className={`absolute inset-0 rounded-2xl pointer-events-none opacity-0 bg-gradient-to-tr ${hoverColorClass} mix-blend-overlay`}
...
/* ═══════════════════════════════════════════════════════
   BACKGROUND EFFECTS
   ═══════════════════════════════════════════════════════ */
function FooterBackground({ mouseX, mouseY }) {
    ...
    return (
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
            ...
            {/* Subtle Auroras tracking mouse */}
            <motion.div
                className="absolute top-[-20%] left-[20%] w-[600px] h-[600px] rounded-full opacity-[0.1] dark:opacity-[0.04]"
                style={{ background: `radial-gradient(circle, var(--color-primary), transparent 70%)`, filter: "blur(100px)", x: useTransform(mouseX, [-0.5, 0.5], ["-30px", "30px"]) }}
            />
            <motion.div
                className="absolute bottom-[-10%] right-[10%] w-[800px] h-[800px] rounded-full opacity-[0.1] dark:opacity-[0.04]"
                style={{ background: `radial-gradient(circle, var(--color-accent), transparent 70%)`, filter: "blur(100px)", x: useTransform(mouseX, [-0.5, 0.5], ["30px", "-30px"]) }}
            />
...
export default function FutureFooter() {
    ...
    return (
        <footer
            ...
            className="relative bg-background text-foreground overflow-hidden pt-10 md:pt-14 lg:pt-20 pb-8 border-t border-slate-200 dark:border-white/5 transition-colors duration-300"
        >
            <FooterBackground mouseX={mouseX} mouseY={mouseY} />

            <div className="relative z-10 max-w-7xl mx-auto px-6">

                {/* ════ PREMIUM GRID (Navigation & Brand) ════ */}
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 lg:gap-8 mb-12 md:mb-16 lg:mb-24">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-20 sm:gap-6 mb-10 md:mb-16 lg:mb-20">
                    <GlassCard delay={0.3} hoverColorClass="from-cyan-400 to-blue-500">
                        <div className="flex flex-col h-full justify-center">
                            <div className="flex items-center gap-3 mb-3">
                                <div className="p-2 rounded-lg bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-gray-400">
                                    <Icons.Email />
                                </div>
                                <span className="text-xs uppercase tracking-widest text-slate-500 dark:text-gray-500 font-semibold">Email Us</span>
                            </div>
                            <span className="text-slate-800 dark:text-white font-medium hover:text-primary transition-colors cursor-pointer text-lg">hello@recenturesoft.com</span>
                        </div>
                    </GlassCard>
                    <GlassCard delay={0.4} hoverColorClass="from-blue-500 to-indigo-500">
                        <div className="flex flex-col h-full justify-center">
                            <div className="flex items-center gap-3 mb-3">
                                <div className="p-2 rounded-lg bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-gray-400">
                                    <Icons.Phone />
                                </div>
                                <span className="text-xs uppercase tracking-widest text-slate-500 dark:text-gray-500 font-semibold">Call Us</span>
                            </div>
                            <span className="text-slate-800 dark:text-white font-medium text-lg">+91 777 000 3288</span>
                        </div>
                    </GlassCard>
                    <GlassCard delay={0.5} hoverColorClass="from-purple-500 to-pink-500">
                        <div className="flex flex-col h-full justify-center">
                            <div className="flex items-center gap-3 mb-3">
                                <div className="p-2 rounded-lg bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-gray-400">
                                    <Icons.MapPin />
                                </div>
                                <span className="text-xs uppercase tracking-widest text-slate-500 dark:text-gray-500 font-semibold">Headquarters</span>
                            </div>
                            <span className="text-slate-800 dark:text-white font-medium text-sm leading-relaxed">A-125, Sector-63,<br />Noida, UP 201301</span>
                        </div>
                    </GlassCard>
                </div>

                {/* ════ PREMIUM BOTTOM BAR ════ */}
                <div className="relative pt-8 flex flex-col md:flex-row justify-between items-center gap-6 text-sm text-slate-500 dark:text-gray-500">
                    <motion.div
