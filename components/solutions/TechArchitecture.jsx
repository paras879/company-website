...
// DataPulse component representing animated data particles
function DesktopDataPulse({ angle, radius, index }) {
    ...
    return (
        <>
            <motion.div
                className="absolute w-2 h-2 rounded-full bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.8)]"
            ...
            />
            <motion.div
                className="absolute w-1.5 h-1.5 rounded-full bg-cyan-400 shadow-[0_0_6px_rgba(34,211,238,0.8)]"
                initial={{ x: 0, y: 0, opacity: 0 }}
                animate={{ x, y, opacity: [0, 1, 1, 0] }}
                transition={{ duration: 2.2, repeat: Infinity, delay: index * 0.35 + 1.1, ease: "linear" }}
                style={{ left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}
            />
        </>
    );
...
export default function TechArchitecture() {
    ...
    return (
        <section className={`${manrope.className} relative w-full py-[clamp(2.5rem,6vw,6rem)] bg-slate-50/50 dark:bg-background transition-colors duration-300 px-4 sm:px-6 lg:px-12 border-t border-slate-200 dark:border-white/5 overflow-hidden`}>
            ...
            <div className="max-w-[1400px] mx-auto flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
                ...
                <div className="hidden lg:block relative w-[48%] h-[550px] flex-shrink-0">
                    ...
                    {/* Flowing Data Lines & Pulse Particles */}
                    {architectureNodes.map((node, idx) => (
                        <div key={`line-${idx}`} className="absolute inset-0 pointer-events-none">
                            <div
                                className="absolute top-1/2 left-1/2 h-[1.5px] bg-gradient-to-r from-indigo-500/20 via-cyan-500/10 to-transparent origin-left"
                                style={{ width: radius, transform: `translateY(-50%) rotate(${node.angle}rad)` }}
                            />
                            <DesktopDataPulse angle={node.angle} radius={radius} index={idx} />
                        </div>
                    ))}
