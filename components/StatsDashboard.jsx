...
/* ═══════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════ */
function MiniDashboard({ data, isPeak }) {
    return (
        <motion.div
            ...
            className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 z-50 w-52"
        >
            <div
            ...
            >
                ...
                {isPeak && (
                    <div
                        className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full text-[10px] font-bold text-white uppercase tracking-wider"
                ...
                {/* Header */}
                <h4 className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: C.primary }}>
...
/* ═══════════════════════════════════════════════════════
   ANIMATED 3D BAR
   ═══════════════════════════════════════════════════════ */
function AnalyticsBar({ data, maxValue, index, isInView, peakValue, chartHeight }) {
    ...
    return (
        ...
        >
            ...
            <AnimatePresence>
                {hovered && <MiniDashboard data={data} isPeak={isPeak} />}
            </AnimatePresence>
            ...
            <div
                ...
                style={{ height: chartHeight, perspective: "400px" }}
                onMouseEnter={() => setHovered(true)}
                onMouseLeave={() => setHovered(false)}
            >
                {/* 3D bar */}
                <motion.div
                    className="relative w-full max-w-[48px] rounded-t-xl cursor-pointer overflow-hidden"
                    style={{
                    ...
                    transition={{
                        height: { delay: index * 0.06 + 0.15, duration: 0.9, ease: [0.22, 1, 0.36, 1] },
                ...
                {isInView && (
                    <motion.div
                        ...
                        transition={{ delay: index * 0.06 + 0.8, duration: 0.4 }}
                    >
                        <div
                        ...
                        {isHighPerformer && (
                            <motion.div
                                ...
                                style={{ border: `1px solid ${C.primary}30` }}
...
/* ═══════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════ */
function TrendLine({ data, maxValue, isInView, chartHeight }) {
...
/* ═══════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════ */
function NetworkLines({ data, maxValue, isInView, chartHeight }) {
    const barCount = data.length;
    const chartH = chartHeight;
...
export default function StatsDashboard() {
    ...
    const countersInView = useInView(countersRef, { once: true, margin: "-100px" });
    ...
    let chartHeight = "260px";
    if (windowWidth < 768) chartHeight = "180px";
    else if (windowWidth < 1024) chartHeight = "220px";
    ...
    const stagger = {
        ...
        visible: {
            opacity: 1,
            y: 0,
            transition: { staggerChildren: 0.1, type: "spring", stiffness: 120 },
        },
    };
    const fadeUp = {
    ...
    return (
        ...
        >
            ...
            {/* ═══ Section header ═══ */}
            <motion.div
                className="relative z-10 text-center max-w-3xl mx-auto px-6 mb-8 md:mb-12"
            ...
            >
                {counters.map((c) => (
                    <motion.div key={c.id} variants={fadeUp}>
                        <Counter3D
                            icon={c.icon}
                            value={countersInView ? c.value : 0}
                            suffix={c.suffix}
                            label={c.label}
                        />
                    </motion.div>
                    ...
                    <div ref={chartRef} className="relative z-10 px-4 md:px-6 lg:px-8 pt-6 md:pt-4 pb-5 md:pb-6 lg:pb-8">
                        ...
                        <div className="relative">
                            ...
                            {[0, 25, 50, 75, 100].map((pct) => (
                                ...
                                >
                                    <span className={`text-[8px] sm:text-[9px] text-slate-400 dark:text-gray-600 tabular-nums w-6 sm:w-8 text-right mr-2 sm:mr-3 -translate-y-1/2 ${pct === 25 || pct === 75 ? 'hidden sm:block' : 'block'}`}>
                            ...
                            <div className="ml-8 sm:ml-11 pb-2 sm:pb-0">
                                <div className="relative w-full">
                                    <NetworkLines data={displayData} maxValue={maxValue} isInView={chartInView} chartHeight={chartHeight} />
                                    <TrendLine data={displayData} maxValue={maxValue} isInView={chartInView} chartHeight={chartHeight} />
